...
class ApiClient {
  ...
  // User endpoints (register/login functionality)
  async registerUser(data: UserCreate): Promise<User> {
...
export const auth = {
  register: (userData: UserCreate) => apiClient.registerUser(userData),
  // Note: The FastAPI backend doesn't have a login endpoint yet
  // You'll need to add JWT authentication to the backend for proper login
};
...
export const users = {
  create: (data: UserCreate) => apiClient.registerUser(data),
